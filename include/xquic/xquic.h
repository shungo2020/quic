...
/* 0: go forward -1: processed, need return*/
typedef int (*xqc_init_packet_filter_pt)(const unsigned char * pkt, size_t* pkt_size, const struct sockaddr *peer_addr, socklen_t peer_addrlen, void *engine_user_data, void *user_data);
...
typedef void (*xqc_conn_custom_udp_pt)(xqc_connection_t *conn, const xqc_cid_t *cid, const char* buf, int size, void *conn_user_data);
...
typedef struct xqc_conn_callbacks_s {
	...
	xqc_conn_custom_udp_pt conn_custom_udp;
...
typedef struct xqc_engine_callback_s {
	...
	xqc_init_packet_filter_pt 		init_packet_filter;
...
XQC_EXPORT_PUBLIC_API
int xqc_conn_fill_dcid(xqc_connection_t* conn, char* buf);
